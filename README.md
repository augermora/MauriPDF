...
The basic single-page viewer uses PDFium through PDFiumCore and supports local PDF opening, previous/next and direct page navigation, 25–500% manual zoom, reset to 100%, Fit Page, Fit Width, and scrolling. Opening and rendering run in the background; rapid navigation uses latest-request-wins behavior and a 64 MiB LRU pixel cache. The previous page stays visible during rendering. Fit modes respond to window resizing; changed sizes use exact-size cached pixels or fresh PDF rendering rather than bitmap scaling. A collapsible, resizable thumbnail sidebar supports visual navigation with lazy visible-range rendering and a separate 8 MiB cache. Full-page prefetching, continuous document scrolling, search, annotations, and editing are not implemented.

Enter a page number and press Enter to navigate, or click a thumbnail. Keyboard shortcuts: Left/Page Up and Right/Page Down for navigation; Home/End for first/last page; Ctrl++ and Ctrl+- for zoom; Ctrl+0 for 100%; Ctrl+O to open; F4 to show/hide thumbnails. While the page-number field is focused, unmodified navigation keys retain their normal text-editing behavior.
