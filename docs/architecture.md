...
On success the viewer deliberately remains backed by the original `SourcePath`; it is not reopened and its caches/session stay valid. `SavedDocumentPath` tracks the last successful output and is the only target of subsequent ordinary Save. The user-facing current path/title uses `SavedDocumentPath` when present and otherwise the source filename. `DocumentEditSession.MarkSavedBaseline` makes the captured current revision clean while retaining history: Undo away from it is dirty and Redo back to it is clean. Later edits are dirty and another successful Save or Save As moves the baseline again. Failed validation/materialization never changes the path or baseline. Consequently clean close needs no warning, while Undo/new edits restore Discard/Cancel protection.
